        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                using (TcpClient client = new TcpClient())
                {
                    ...
                    // Receive response
                    byte[] buffer = new byte[1024 * 1024]; // 1MB buffer for large schema data
                    ...
                    try
                    {
                        DatabaseSchema schema = JsonConvert.DeserializeObject<DatabaseSchema>(response);
